...
namespace Mint.Parse
{
    public class Ast<T> : IEnumerable<Ast<T>>
    {
        ...
        // Append operator
        public static Ast<T> operator +(Ast<T> left, Ast<T> right)
            => left?.Append(right) ?? right;


        public static explicit operator Ast<T>(T value)
            => new Ast<T>(value);


        public IEnumerator<Ast<T>> GetEnumerator()
            => List.GetEnumerator();
